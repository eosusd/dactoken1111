...
namespace eosdac {
    ...
    void eosdactokens::lock(asset lock) {
        eosio_assert(lock.symbol.is_valid(), "ERR::UNLOCK_INVALID_SYMBOL::invalid symbol name");
        auto sym_name = lock.symbol.code().raw();
        stats statstable(_self, sym_name);
        auto token = statstable.find(sym_name);
        eosio_assert(token != statstable.end(), "ERR::UNLOCK_NON_EXISTING_SYMBOL::token with symbol does not exist, create token before lock");
        const auto &st = *token;
        require_auth(st.issuer);

        statstable.modify(st, name{}, [&](auto &s) {
            s.transfer_locked = true;
        });
    }
